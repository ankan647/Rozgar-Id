...
export default function IssuerLogin() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', phone: '', type: 'training_institute', state: '', city: '', registrationNumber: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = tab === 'login' ? '/auth/issuer/login' : '/auth/issuer/register'
      const payload = tab === 'login' ? { email: form.email, password: form.password } : form
      const { data } = await api.post(endpoint, payload)
      login({ ...data.issuer, role: 'issuer' }, { accessToken: data.accessToken, refreshToken: data.refreshToken })
    ...
    <div className="min-h-screen bg-gradient-to-br from-surface via-brand-50 to-brand-100 flex items-center justify-center px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-gradient-to-br from-brand-700 to-brand-900 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-brand">
            <Building2 className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-2xl font-bold text-brand-900">{t('home.issuer_portal')}</h1>
          <p className="text-brand-600 text-sm mt-1">{t('home.issuer_desc')}</p>
        </div>
